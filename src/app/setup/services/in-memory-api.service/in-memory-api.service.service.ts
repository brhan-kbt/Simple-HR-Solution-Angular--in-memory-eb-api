...
export class InMemoryApiServiceService  implements InMemoryDbService {
  createDb() {
    const candidates = [
    ...
    const employees = [
      { id: 1, name: 'John', designation: 'Manager', deptId: 1, salaryId: 1 },
      { id: 2, name: 'Jane', designation: 'Developer', deptId: 1, salaryId: 2 },
      { id: 3, name: 'Bob', designation: 'Manager', deptId: 2, salaryId: 3 },
    ];
  ...
  genId(candidates: { id: number, name: string, email: string, phone: string }[]): number {
    return candidates.length > 0 ? Math.max(...candidates.map(candidate => candidate.id)) + 1 : 1;
  }
